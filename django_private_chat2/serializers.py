...
def serialize_message_model(m: MessageModel, user_id):
    ...
    obj = {
        "id": m.id,
        "text": m.text,
        "sent": datetime.fromtimestamp(int(m.created.timestamp())) ,
        "edited": datetime.fromtimestamp(int(m.modified.timestamp())),
        "read": m.read,
...
def serialize_dialog_model(m: DialogsModel, user_id):
    ...
    obj = {
        "id": m.id,
        "created": datetime.fromtimestamp(int(m.created.timestamp())),
        "modified": datetime.fromtimestamp(int(m.modified.timestamp())),
